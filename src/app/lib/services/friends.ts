...
export const getAllFriends = async (user: string) => {
  ...
  try {
    const friends = await client.execute({
      ...
      u.email AS email,
      GROUP_CONCAT(DISTINCT ug.group_id) AS shared_groups_ids,
      GROUP_CONCAT(DISTINCT g.group_name) AS shared_groups_names
    FROM
      users u
    JOIN
    ...
    GROUP BY
      u.user_id
    HAVING
      COUNT(DISTINCT ug.group_id) = (SELECT COUNT(DISTINCT group_id) FROM user_group WHERE user_id = ?)
    `,
      args: [user, user, user],
    });
